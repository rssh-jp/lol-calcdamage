...
var CONST_CHAMP_LIST = {
    ALL    : { CACHE_NAME : CACHE_NAME.CHAMPION_ALL_ALL,    EXTERNAL_FUNC : 'searchChampDataAll',    INNER_FUNC : searchChampListAll   },
    INFO   : { CACHE_NAME : CACHE_NAME.CHAMPION_INFO_ALL,   EXTERNAL_FUNC : 'searchChampDataInfo',   INNER_FUNC : searchChampListInfo  },
    STATS  : { CACHE_NAME : CACHE_NAME.CHAMPION_STATS_ALL,  EXTERNAL_FUNC : 'searchChampDataStats',  INNER_FUNC : searchChampListStats },
    SPELLS : { CACHE_NAME : CACHE_NAME.CHAMPION_SPELLS_ALL, EXTERNAL_FUNC : 'searchChampDataSpells', INNER_FUNC : searchChampListSpells},
};
...
var searchChampList = function(type, callback){
    ...
    // apiから取得
    task.push(function(next){
        RequestMan[CONST_CHAMP_LIST[type].EXTERNAL_FUNC](null, function(err, res){
            if(err){
                next(err);
                return;
            }
